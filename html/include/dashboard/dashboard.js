(function ($, window, document) {
  ...
  const rollHistoryEndpoint =
    window.DASHBOARD_ROLL_HISTORY_ENDPOINT || '/include/dashboard/roll_history_act.php';
  const ROLL_HISTORY_REFRESH_MS = 30000;
  ...
  function syncRollHistoryRangeInputs() {
    if (!$rollHistoryRangeStart.length || !$rollHistoryRangeEnd.length || !hasValidRollHistoryRange()) {
      return;
    }
  ...
  function rollDurationMinutes(durationSeconds) {
    return Math.max(1, Math.round(toNum(durationSeconds) / 60));
  }
  ...
  function rollDurationWidth(durationSeconds) {
    const minutes = rollDurationMinutes(durationSeconds);
    return Math.max(18, Math.round(minutes * TIMELINE_PIXELS_PER_MINUTE));
  }
  ...
  function renderRealtimeTimeAxis(timelineWidth, rangeStartMs, rangeEndMs) {
    ...
    const labelStepMs = TIMELINE_LABEL_INTERVAL_MINUTES * 60 * 1000;
    const firstTickTimeMs = Math.ceil(safeRangeStartMs / tickStepMs) * tickStepMs;
    let majorTickCount = 0;

    for (
      let tickTimeMs = firstTickTimeMs;
      tickTimeMs <= safeRangeEndMs;
      tickTimeMs += tickStepMs
    ) {
      ...
      const isMajor = tickDate.getMinutes() % 60 === new Date(safeRangeEndMs).getMinutes() % 60;
      let edgeClass = '';
  ...
  function setRollHistoryLoading(isLoading, text) {
    ...
    $rollHistoryLoading.text(text || 'Memuat roll history...');
  ...
  function showRollHistoryError(msg) {
    const message = msg || 'Gagal memuat roll history.';
  ...
  function renderRollTimeline(rolls, rangeStartMs, rangeEndMs) {
    ...
    orderedRolls.forEach(function (roll) {
      ...
      const left = Math.max(
        0,
        Math.round(
          ((clampedStartMs - safeRangeStartMs) / Math.max(1, safeRangeEndMs - safeRangeStartMs)) *
            timelineWidth
        )
      );
      const width = Math.max(
        8,
        Math.round(
          ((clampedEndMs - clampedStartMs) / Math.max(1, safeRangeEndMs - safeRangeStartMs)) *
            timelineWidth
        )
      );
      ...
      const activeClass = String(roll.id) === String(selectedRollId) ? ' is-active' : '';
      const statusClass = isShutdown ? ' is-status-shutdown' : '';
      const compactClass = width < TIMELINE_INLINE_TEXT_MIN_WIDTH ? ' is-compact' : '';
      const showInlineText = width >= TIMELINE_INLINE_TEXT_MIN_WIDTH;
  ...
  function setSelectedRoll(rollId) {
    ...
    $rollHistoryTimeline.find('.roll-history-item').each(function () {
      const $item = $(this);
      $item.toggleClass('is-active', String($item.attr('data-roll-id')) === String(rollId));
    });
  }
  ...
  function fetchRollHistory(preferredRollId) {
    ...
    if (!lastRenderedRollHistorySignature) {
      setRollHistoryLoading(true, 'Memuat roll history...');
    }
    ...
    currentRollHistoryRequest.done(function (res) {
      if (!res || !res.ok) {
        showRollHistoryError(
          res && res.message ? res.message : 'Response roll history tidak valid.'
        );
        return;
      }
  ...
  function applyRollHistoryRangeFromInputs() {
    ...
    if (!Number.isFinite(nextStartMs) || !Number.isFinite(nextEndMs) || nextEndMs <= nextStartMs) {
      syncRollHistoryRangeInputs();
      showRollHistoryError('Range waktu roll history tidak valid.');
      return;
    }
