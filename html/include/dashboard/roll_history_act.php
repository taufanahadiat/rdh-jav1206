...
): ?array
{
  ...
  $durationSeconds = roll_history_duration_seconds($starttime, $endtime);
  if ($rangeStart !== null && $now < $rangeStart) {
    return null;
  }
  if ($rangeEnd !== null) {
    try {
...
try {
  ...
  if (($selectedRollId <= 0 || !$selectedRollExistsInRange) && !empty($rolls)) {
    $selectedRollId = 0;
    foreach ($rolls as $rollItem) {
      $candidateId = (int) ($rollItem['id'] ?? 0);
      if ($candidateId > 0) {
        $selectedRollId = $candidateId;
        break;
      }
    }
  }
